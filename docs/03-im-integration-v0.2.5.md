# 个人工作台 IM 消息接入技术方案 v0.2.5 (Final Freeze Closure · 终审签字版)

> 项目：个人工作台 (Personal AI Workspace) · 模块：统一消息中心 (Unified IM Hub)  
> 日期：2026-09-04 · 状态：Final Freeze Sign-off Version (Resolved P1-IM-6-R1, P1-IM-6-R2, P1-IM-7-R1 per Sol Audit)
...
- **批次去重与事务执行算法 (Batch Ingestion Algorithm)**：
  ...
  FOR EACH record IN batch:
      ...
      IF record.provided_digest IS NOT NULL AND record.provided_digest != server_digest:
          ...
          RAISE BadRequestError("Provided digest mismatch with server canonical digest")
...
- **Resync Snapshot 穷尽分页完成门禁契约 (Exhaustive Completion Invariant)**：
  ...
  **详细执行流程**：
  ...
  3. 前端发起 Timeline 分页加载，携带参数：
     ...
     ```
     服务端强制执行栅栏过滤：`WHERE ingest_seq <= 2000`；前端循环遍历拉取，直到 `next_cursor === null`，确认快照内全量历史已彻底加载；
...
2. **[AT-2: 跨摄入路径规范化 Digest 一致性]** 微信同一消息分别经由 SSE 格式与 `/timeline` 格式摄入，验证转换后的 `CanonicalIMPayloadV1` UTF-8 字节逐字节相等，算出的 `server_digest` 与 `dedupe_key` 100% 相同，成功触发幂等跳过；
...
6. **[AT-6: 开区间回放与双游标优先级]** 请求携带 `?after_seq=1000` 与 `Last-Event-ID: 1100`，验证服务端从开区间 `> 1100` 起始回放，不产生重复回放与误 resync；future cursor 传入报错 400；
7. **[AT-7: Resync 穷尽分页与生命周期对抗测试]** 待补消息总数 137 条、单页 `limit=50`，验证客户端收到 `resync_required` 后立即 `close()` 旧连接，连续分页 3 次直到 `next_cursor == null`，之后重新建立 SSE 连接，达成 0 消息遗漏；
