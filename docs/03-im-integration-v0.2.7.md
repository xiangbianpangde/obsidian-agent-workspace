# 个人工作台 IM 消息接入技术方案 v0.2.7 (Final Freeze Closure · 终审签字版)

> 项目：个人工作台 (Personal AI Workspace) · 模块：统一消息中心 (Unified IM Hub)  
> 日期：2026-09-04 · 状态：Final Freeze Sign-off Version (Resolved P1-IM-6-R4 & P1-IM-6-R3-T1 per Sol Audit)
...
- **规范化载荷事实模型 (`CanonicalIMPayloadV1`) 与 `reply_to` 身份域硬契约**：
  ```typescript
  ...
  ```
  **`reply_to` 身份域硬契约 (Reply-To Identity Domain Invariant)**：
  > **`CanonicalIMPayloadV1.reply_to` MUST be `null` or a stable source-side reply-target physical/message locator (or a deterministic encoding thereof) within the source identity domain. It MUST NOT contain `IMMessageItem.id`, `ingest_seq`, database primary keys, workspace-generated opaque IDs, path-specific event IDs, or any other locally derived identifier. The same objective reply relationship observed through realtime and replay paths MUST canonicalize to exactly the same `reply_to` value. If a source cannot provide or deterministically recover such a stable locator across all supported ingestion paths, `reply_to` MUST canonicalize to `null` consistently for those paths.**
...
2. **[AT-2: 跨摄入路径规范化 Digest 一致性与 reply_to 身份域完整性]** 
   - **正向一致性**：微信同一回复消息分别经由 SSE 格式与 `/timeline` 格式摄入，验证转换后的 `CanonicalIMPayloadV1.reply_to` 均指向同一源端定位符，字节逐字节相等，算出的 `server_digest` 与 `dedupe_key` 100% 相同，成功触发幂等跳过；
   - **负向敏感性**：注入相同 `dedupe_key` 但不同源端 `reply_to` 的记录，验证计算出不同 `server_digest` 并触发 `IdentityConflictError`；
...
5. **[AT-5: 身份域不一致双分支阻断 (P1-IM-6-R3-T1)]** 
   - **分支 A (source mismatch)**：提交 `record.source = "wechat"` 但 `message.source = "qq"` 的记录，验证返回 `400 InvalidIngestEnvelope`，写入 0 条，Watermark 不推进；
   - **分支 B (account_id mismatch)**：提交 `record.source = message.source = "wechat"`，但 `record.account_id = "acc_1"` 且 `message.account_id = "acc_2"` 的记录，验证返回 `400 InvalidIngestEnvelope`，写入 0 条，Watermark 不推进；
